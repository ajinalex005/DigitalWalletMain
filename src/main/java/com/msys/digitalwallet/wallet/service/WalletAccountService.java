...
//                                    wallet.setStatus(virtualAccountResponse.getStatus().name());
    ...
    public Mono<ApiWalletTransferResponse> initiateWalletTransfer(String userId, ApiWalletTransferRequest apiWalletTransferRequest) {
        ...
        String fromUserEmail = getUserEmail(fromUserWalletAccount.getUserId());
        String toUserEmail = getUserEmail(toUserWalletAccount.getUserId());
        Transfer transfer = transferMapper.toTransfer(userId, transactionId, fromUserWalletAccount.getId().toString(), fromUserEmail,
                toUserWalletAccount.getId().toString(), toUserEmail, apiWalletTransferRequest.getAmount(), apiWalletTransferRequest.getCurrency(),
                apiWalletTransferRequest.getReason(), "Success");
     ...
     private String getUserEmail(String userId){
         ...
         if (userOptional.isEmpty())
             throw new BusinessException(
                     ...
                     String.format("Given UserId(%s) is not valid", userId));

         return userOptional.get().getEmailAddress();
     }
    ...
    public Mono<HashMap<String,Object>> executeWalletToBankTransfer(String userId,
        ...
        Mono<HashMap<String,Object>> dbEntryMapMono = walletAccount
                .doOnNext(apiUserWalletAccount ->
                    ...
                    String fromUserEmail = getUserEmail(userId);
                    String toUserEmail = null;
                    //Create Transfer data
                    Transfer transfer = createTransfer(walletTransactionExecuteRequest,userWalletAccount,userId, walletId,
                            fromAccountTYpe, fromUserEmail,bankAccountNo, toAccountTYpe, toUserEmail, journalEntry);
    ...
    public Mono<HashMap<String,Object>> executeBankToWalletTransfer(String userId,
        ...
        Mono<HashMap<String,Object>> dbEntryMapMono = walletAccount
                .flatMap((UserWalletAccount userWalletAccount) ->{
                    ...
                    String toUserEmail = getUserEmail(userId);;
                    //Create Transfer data
                    Transfer transfer = createTransfer(walletTransactionExecuteRequest,userWalletAccount,userId, bankAccountNo,
                            fromAccountTYpe, fromUserEmail, walletId, toAccountTYpe, toUserEmail, journalEntry);
                    dbEntryMap.put("TRANSFER",transfer);
                    // Updating wallet balance after transaction
                    BigDecimal walletBalance = userWalletAccount.getAccountBalance();
 ...
 private Transfer createTransfer(WalletTransactionExecuteRequest walletTransactionExecuteRequest, UserWalletAccount userWalletAccount,
                                 String userId, String fromAccount, String fromAccountTYpe, String fromUserEmail, String toAccount,
                                 String toAccountTYpe, String toUserEmail, JournalEntry journalEntry) {
     Double requestedAmount = walletTransactionExecuteRequest.getAmount().getValue();
     LocalDateTime transactionDateTime = LocalDateTime.now();
